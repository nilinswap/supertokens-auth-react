...
function normaliseEmailPasswordConfig(config) {
    ...
    return __assign(__assign({}, (0, utils_1.normaliseAuthRecipeWithEmailVerificationConfig)(config)), { signInAndUpFeature: signInAndUpFeature, resetPasswordUsingTokenFeature: resetPasswordUsingTokenFeature, override: override });
}
    ...
    var defaultSignInFields = signUpForm.formFields.reduce(function (signInFieldsAccumulator, field) {
        ...
        if (field.id === "password") {
            return __spreadArray(__spreadArray([], signInFieldsAccumulator, true), [
                __assign(__assign({}, field), { autoComplete: "current-password", validate: validators_1.defaultLoginPasswordValidator }),
            ], false);
        }
